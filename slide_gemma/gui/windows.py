"""Window enumeration helpers (Windows-only)."""
...
class WindowDescriptor:
    ...
    def formatted_title(self, ui_language: str = "en") -> str:
        text = self.base_title
        if self.is_minimized:
            suffix = " [Minimized]" if normalize_ui_language(ui_language) != "zh" else " [已最小化]"
...
def list_windows(excluded_hwnds: set[int] | None = None) -> list[WindowDescriptor]:
    ...
    def callback(hwnd, _lparam):
        ...
        windows.append(WindowDescriptor(
            hwnd=hwnd_int, title=title, process_name=process_name,
